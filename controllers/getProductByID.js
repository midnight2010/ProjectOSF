...
function getProductByID(req,res) {
   ...
   if(variants.length > 0)
    if(variants[0].variation_values?.size) {
        variants.sort((a,b) => a.variation_values?.size - b.variation_values?.size)
    }
    else if(variants[0].variation_values?.accessorySize){
        variants.sort((a,b) => a.variation_values?.accessorySize - b.variation_values?.accessorySize)
    } 
        let arr = result.data[0].primary_category_id.split('-');
        let breadcrumbs = helper(arr)
        breadcrumbs[('_' + id)] = '/' + id;
        variants = variants ? variants : '';
        res.render('productDescription',{product:result.data,breadcrumbs,id:id,variants})
     })
