...
async function getProduct(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    results.push(
      axios(
        `${process.env.URL}/products/product_search?id=${items[i].productId}&secretKey=${process.env.SECRET_KEY}`
      ).then(({ data }) => {
        const { name, page_description, id } = data[0];
        let link = data[0].image_groups.find(
          (element) => element.view_type == "small"
        ).images[0].link;
